      ...
      <v-layout wrap align-center justify-center row>
        <v-flex xs12>
          <v-img :src="imgPath.src" class="elevation-1 hidden-sm-and-up">
            ...
            >
              <div class="google-font font-weight-light" style="font-size:200%">
                Agenda
              </div>
            </v-layout>
          </v-img>
        ...
        <v-flex xs12 md9 sm10 lg10 class="pa-0 mt-5 hidden-sm-and-down">
          <span
            ...
            style="border-radius:8px;background-color:#d1c4e9;color:#311b92;font-size:150%"
            >Agenda</span
          >
          <br /><br />
          <p class="google-font" style="font-size:120%">
            Hone your skills. Ask technical questions. Get inspired. This year’s
            program is packed with amazing content from amazing speakers.
            <br />
            Check back often as we continue to add new sessions.
          </p>
        </v-flex>
                    ...
                    <v-card-text>
                      <v-layout justify-start row fill-height wrap>
                        ...
                        >
                          <v-card
                            class="elevation-0"
                            style="border: 1px solid rgb(224, 224, 224);min-height:280px;border-radius:5px"
                          >
                            <v-card-title primary-title>
                              <div>
                                <p
                                  class="mb-0 google-font"
                                  style="font-size:130%;color:#424242"
                                >
                                  {{ item.title }}
                                </p>
                                <v-chip
                                  v-if="item.track.length > 0"
                                  label
                                  small
                                  class="ma-0 my-1"
                                  outline
                                  :color="showColor(item.track)"
                                >
                                  <v-icon small left>{{
                                    showIcon(item.track)
                                  }}</v-icon>
                                ...
                                <div class="google-font" style="font-size:110%">
                                  {{ item.des }}
                                </div>
                              </div>
                            </v-card-title>
                    ...
                    <v-card-text>
                      <v-container style="max-width: 900px;">
                        <v-timeline
                          ...
                          class=""
                        >
                          <v-timeline-item :color="showColor(item.tag)" small>
                            <v-layout pt-3>
                              <v-flex xs3>
                                <strong class="google-font">{{
                                  item.startTime
                                }}</strong>
                              </v-flex>
                              <v-flex xs9>
  ...
  methods: {
    showColor(tag) {
      if (tag == 'Web') {
        ...
        return 'green'
      } else if (tag == 'Soft-Skills') {
        return '#4a148c'
      } else if (tag == 'Fun') {
        return '#9e9d24'
      } else if (tag == 'Tech-Talk') {
        return 'orange'
      }
    },
    showIcon(tag) {
